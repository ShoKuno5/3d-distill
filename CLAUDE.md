...
- **Experiment reports**: Save to `distill_methods/reports/` as **self-contained HTML** (images base64-embedded, viewable offline). Naming: `YYYY-MM-DD_<topic>.html`. Related reports form a series — each links to the previous with a "Previous" header. Create a report at experiment start with design and hypotheses; fill in results after completion. Use `/report-experiment [focus]` to generate. Reports are agentic — Claude reads all logs, config, and code to write analysis, not mechanical parsing.
