#!/usr/bin/env python3
"""Block-level inference time profiling for FlashVDM (Hunyuan3D-2.0 turbo).

Loads turbo DiT pipeline with FlashVDM enabled, expands __call__ inline
with torch.cuda.synchronize() + time.perf_counter() around each block.
...
Usage (from models/hunyuan3d):
    ...
    ../../experiments/distill_profiling/run_profile_flashvdm.py \
...
def profile_one_sample(pipeline, image, num_inference_steps, octree_resolution,
                       num_chunks, generator):
    """Run FlashVDM pipeline with block-level timing.

    Replicates Hunyuan3DDiTFlowMatchingPipeline.__call__() with timers.
    """
    import numpy as np
    import torch
    ...
    # --- Diffusion sampling loop (5 steps) ---
    t0 = sync_and_time()
    ...
    # --- Marching cubes (FlashVDM accelerated) ---
    t0 = sync_and_time()
...
def main():
    parser = argparse.ArgumentParser(description="FlashVDM block-level profiling")
    ...
    if model_cfg is None:
        print("ERROR: 'flashvdm' not found in config")
        sys.exit(1)
    ...
    print(f"FlashVDM profiling | steps={num_inference_steps} "
          f"octree_res={octree_resolution} num_chunks={num_chunks} | "
          f"{len(samples)} samples")

    # Import from hunyuan3d v2.0 (models/hunyuan3d)
    import torch
    ...
    # Load turbo DiT pipeline + FlashVDM
    ...
    if samples:
        ...
        with torch.inference_mode():
            profile_one_sample(
                pipeline, warmup_img, num_inference_steps,
