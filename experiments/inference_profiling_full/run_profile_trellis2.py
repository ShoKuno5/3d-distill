#!/usr/bin/env python3
"""Block-level inference time profiling for TRELLIS.2 (v2 — finer decode split).

Splits the former 'decode' block into decode_shape, decode_tex, and postprocess
to isolate shape vs texture decode costs. Always saves mesh OBJ.

Usage (from models/trellis2):
    PYTHONPATH=. CUDA_HOME=../../envs/cuda-12.8 \
    ../../envs/miniconda3/envs/trellis2/bin/python \
    ../../experiments/inference_profiling_full/run_profile_trellis2.py \
    --config ../../experiments/inference_profiling_full/config.yaml
"""
...
def profile_one_sample(pipeline, image, seed, pipeline_type):
    ...
    v2: decode is split into decode_shape, decode_tex, and postprocess.
    """
    import torch
    ...
    # --- Decode shape (Sparse U-Net -> FDG mesh extraction) ---
    t0 = sync_and_time()
    meshes, subs = pipeline.decode_shape_slat(shape_slat, res)
    timings["decode_shape"] = sync_and_time() - t0

    # --- Decode texture (Sparse U-Net -> texture voxels) ---
    t0 = sync_and_time()
    tex_voxels = pipeline.decode_tex_slat(tex_slat, subs)
    timings["decode_tex"] = sync_and_time() - t0

    # --- Postprocess (fill_holes + MeshWithVoxel construction) ---
    t0 = sync_and_time()
    out_mesh = []
...
def main():
    parser = argparse.ArgumentParser(description="Trellis2 block-level profiling (v2)")
    ...
    print(f"TRELLIS.2 profiling (v2) | pipeline_type={pipeline_type} | {len(samples)} samples")
