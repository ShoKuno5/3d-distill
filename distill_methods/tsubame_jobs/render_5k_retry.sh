...
#$ -N render-5k-retry
#$ -j y
#$ -o /gs/fs/tga-koike-shanda2/sk/scratch/tsubame_logs/render_5k_retry.qsub.log
#
...
export MANIFEST="${MANIFEST:-$REPO/distill_methods/manifests/5k_balanced/retry_517.csv}"
    ...
    mpirun -n "$NNODES" --map-by ppr:1:node --hostfile "$OMPI_HOSTFILE" \
        ...
        bash -c "
            ...
            for L in \$(seq 0 \$((NPROC_PER_NODE - 1))); do
                ...
                LOG=\"\$TSUBAME_ROOT/scratch/tsubame_logs/render_5k_retry_shard\${SHARD}.log\"
