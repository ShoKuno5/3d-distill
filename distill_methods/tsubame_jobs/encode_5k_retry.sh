...
#$ -N encode-5k-retry
#$ -j y
#$ -o /gs/fs/tga-koike-shanda2/sk/scratch/tsubame_logs/encode_5k_retry.qsub.log
#
...
export CFG="$REPO/distill_methods/configs/config_5k_m2_tsubame_retry.yaml"
    ...
    mpirun -n "$NNODES" --map-by ppr:1:node --hostfile "$OMPI_HOSTFILE" \
        ...
        bash -c "
            ...
            for L in \$(seq 0 \$((NPROC_PER_NODE - 1))); do
                ...
                LOG=\"\$TSUBAME_ROOT/scratch/tsubame_logs/encode_5k_retry_shard\${SHARD}.log\"
