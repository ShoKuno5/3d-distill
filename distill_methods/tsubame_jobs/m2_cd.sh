...
#$ -N m2cd-tsubame
#$ -j y
#$ -o /gs/fs/tga-koike-shanda2/sk/scratch/tsubame_logs/m2_cd.qsub.log
#
# TSUBAME M2 CD (Consistency Distillation) training on H100 96GB node_f (4 GPU).
#   - num_timesteps=50, single-stage, 15000 steps
#
# Submit: qsub -ar 6925 -g tga-koike-shanda m2_cd.sh

source "${COMMON_SH:-/gs/fs/tga-koike-shanda2/sk/3d-distill/distill_methods/tsubame_jobs/_common.sh}"
tsubame_log_init "m2_cd"
tsubame_setup_env

RUN_NAME="m2_cd_tsubame_$(date +%Y%m%d_%H%M)"
...
tsubame_print_banner "M2 CD" "$RUN_NAME" "$OUTPUT_ROOT"
