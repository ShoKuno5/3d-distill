#!/usr/bin/env bash
# qzcli M1-fast variant: 2-GPU DDP with halved total_steps.
#
# Total compute = 2 GPU × 7500 step × batch=4 = 60K sample-passes,
# matches DSW (1 GPU × 15000 × 4). Effective batch=8 (DDP), grad noise
# halved. Wall time ~half of M1-base (1 GPU × 15K).
#
# Original M1
#
# Setup:
...
RUN_NAME="m1_dmd2_fast_$(date +%Y%m%d_%H%M)"
...
cfg['training']['total_steps'] = 7500  # M1-fast: halved, 2-GPU DDP doubles effective batch
