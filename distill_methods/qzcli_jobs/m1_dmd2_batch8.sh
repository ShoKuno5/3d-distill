#!/usr/bin/env bash
# qzcli M1 batch=8 challenge run: test if batch=8 fits on 1x H200 141GB.
#
# If OOM at early step (within first 100), kill and fall back to M1-fast
# (2-GPU DDP). If survives 500+ steps, we know 1 GPU + batch=8 works ->
# can use this config for M2-M5 (cheaper than DDP).
#
# Total compute matches DSW: 1 GPU x 7500 step x batch=8 = 60K sample
# passes = DSW (1 GPU x 15000 x 4).
#
# Original M1
#
# Setup:
...
RUN_NAME="m1_dmd2_batch8_$(date +%Y%m%d_%H%M)"
...
cfg['training']['total_steps'] = 7500  # batch=8: half steps for matched total samples
